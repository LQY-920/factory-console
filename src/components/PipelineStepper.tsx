      ...
      {steps.map((step, index) => {
        ...
        return <div className={`pipeline-step pipeline-step--${step.state} pipeline-step--id-${step.id}`} key={step.id}>
          ...
          <div className="pipeline-step__row">
            <span className="pipeline-step__icon"><Icon size={28} strokeWidth={1.8} />{step.count ? <em>{step.count}</em> : null}</span>
          ...
          {step.detail ? <code className="branch-chip">{step.detail}</code> : null}
        </div>
      })}
    </div>
