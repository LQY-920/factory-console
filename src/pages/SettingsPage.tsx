...
export function SettingsPage() {
  ...
  return <div className="page-stack"><div className="page-heading"><div><h1>{t('settings.title')}</h1></div></div><div className="settings-grid">
    <Panel title={t('settings.language')}><div className="setting-row"><Languages size={22} /><div className="segmented"><button className={i18n.language === 'zh-CN' ? 'active' : ''} onClick={() => setLocale('zh-CN')}>{t('languages.zh')}</button><button className={i18n.language === 'en-US' ? 'active' : ''} onClick={() => setLocale('en-US')}>{t('languages.en')}</button></div></div></Panel>
